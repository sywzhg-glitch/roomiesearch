 ...
 * Ranking Algorithm
 *
 * Score = (avg_rating * 20) + (interested_pct * 30) + (applying_pct * 50)
 * Tie-breaker: most recent activity
 *
 * avg_rating: mean of all member ratings (1-5) → scaled to 0-20
 * interested_pct: % of members who marked "interested" → 0-30
 * applying_pct: % of members who marked "applying" → 0-50
 */
...
export function rankListings(
  ...
  // Compute scores and avg ratings
  ...
  // Compute final scores after setting avgRating
  const scored = withScores.map((l) => ({
    ...l,
    score: computeScore(l, memberCount),
  }));

  // Sort: score desc, then avgRating desc, then recency desc
  scored.sort((a, b) => {
